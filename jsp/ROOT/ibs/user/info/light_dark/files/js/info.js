...
function paintSelects(d) {
	var cur_sel = "<option value=''>Select:</option>", cur_sel2 = "<option value=''>Select:</option>";
	var cb_cur = d.data[0];
	for (var i = 0; i < cb_cur.children_curr.length; i++) {
		cur_sel += `<option value="`+ cb_cur.children_curr[i].curr_char_code +`">` + cb_cur.children_curr[i].curr_name + `</option>`;
		if(cb_cur.children_curr[i].curr_char_code=="USD") {
			cur_sel2 += `<option value="`+ cb_cur.children_curr[i].curr_char_code +`" selected >` + cb_cur.children_curr[i].curr_name + `</option>`;
		} else {
			cur_sel2 += `<option value="`+ cb_cur.children_curr[i].curr_char_code +`" >` + cb_cur.children_curr[i].curr_name + `</option>`;
		}		
	}
	getDOM("currency-select").innerHTML = cur_sel;
	getDOM("currency-select2").innerHTML = cur_sel2;
}
...
function getCalendar(param) {
	...
	if(is.undef(param.oper_day)) {
		param.oper_day= getDOMValue("cur_day");
	}
	AJAX.load({
		POST: param,
		onSuccess: function (d) {
...
function drawBranch(data) {
	...
	if(!data || is.undef(data.length)) return;
		...
		html += `<div class="cdc-item" has-cross="` + cross_infos.length + `">
					...
					<div>
						...
						<div class="right">
							...
							<img src="files/icons/` + getChrtImg(c_b_r, d_b_r) + `.svg" />
						</div>
					</div>
					<div>
						<div class="left">
						...
						<div class="right">
							...
							<img src="files/icons/` + getChrtImg(c_s_r, d_s_r) + `.svg" />
						</div>
					</div>
				</div>`;
	}
...
function currencyCalc(el) {
	...
	let str = `<div class="currency-calc" style="top:`+ size(el.offsetTop) +`px; left:` + size(el.offsetLeft) + `px">
		<div class="cc-body">
			<div class="ccb-close">
...
function changeCurImg(t) {
	...
	if (t == "custom-select") {
		...
		$("#currency-img").css({width: size(30),height: size(30)});
		if(selectCurr)
		    drawCurrInfo(cur);
