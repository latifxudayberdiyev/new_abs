...
<div class="ipanel">
	...
	<div class="ipanel-body">
		...
		<div class="ipb-item bankinfo">
			<div class="ipb-title">
				<div class="ipb-icon">
					<i class="fas fa-globe-americas"></i>
				</div>
				<div class="ipb-text">
					<%= lang.get(si_info_branch) %>
					<div class="ipb-address">
						<i class="fas fa-map-marker-alt"></i>
						<ul class="viloyat">
						  <%= stored.execSelect("select '<li data-state=\"region_code_'||t.CODE||'\"'||decode(t.CODE,core_info.get_curr_region,' class=\"active\"','')||'>'||t.Name||'</li>' from v_region t")%>
						</ul>
					</div>
				</div>
			</div>
			<div class="ipb-content">
				<div class="svgmap">
					<%@include file="files/img/map.svg" %>
				</div>
				<div class="svginfo" id="svginfo"></div>
			</div>
		</div>
	...
	<div class="ipanel-bottom">
		<div class="chart-box" id="chart_box">
				...
				<div class="chbt-select">
					...
					<div class="cdt-title">
						<div class="custom-select2">
							<select id="currency-select2"></select>
						</div>
					</div>
				</div>
				<div class="chbt-date">
			...
			<div class="chart-box-body">
				<div class="chart-radio" id="chart_radio"></div>
...
<div class="currency-data md-effect">
	<div class="cd-box">
		...
		<div class="cd-content">
			<div class="cdc" id="curr_content"></div>
			<form>
				<div class="button-group" id="end_sum_btn"></div>
			</form>			
			<div class="cdc" id="cross_curs"></div>
		</div>
		<div class="cd-bottom">
...
<script src="files/js/chart/fusioncharts.theme.candy.js"></script>
...
<%
	...
	try {
		...
		cs.execute();
		out.print("<script> curr_data=eval(\"("+ cs.getStringResult() +")\"); drawCurrency(curr_data); </script>");
	} catch (Exception ex) {
	...
	try {
		...
		cs.setAllParameters("Request");
		cs.execute();
		out.print("<script> work_data=eval(\"("+ cs.getStringResult() +")\"); drawWorkingTime(work_data); </script>");
	} catch (Exception ex) {
		response.setHeader("RT", "error");
		out.print(Util.getUserMessage(ex));
	}
	...
	try {
		...
		cs.setString("i_Region_Code", "26");
		cs.execute();
		out.print("<script> svgInfo(" + cs.getStringResult() + "); </script>");
	} catch (Exception ex) {
		response.setHeader("RT", "error");
		out.print(Util.getUserMessage(ex));
	}
	...
	try{
	 CallableStatement cs = conn.prepareCall("{? = call Core_info.Get_Info_For_Chart}");
	 cs.registerOutParameter(1, Types.CLOB);
	 cs.execute();
	 Clob responseBodyClob = cs.getClob(1);
	 String responseBody = responseBodyClob.getSubString(1, (int) responseBodyClob.length());
	 		out.print("<script> drawChartData("+ responseBody +"); </script>");
  } catch(Exception ex){
		response.setHeader("RT", "error");
		out.print(Util.getUserMessage(ex));
  }
