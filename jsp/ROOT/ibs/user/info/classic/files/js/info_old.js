...
function drawBranch(data) {
	...
	if(!hasData){
		code=`<p class="no_data_found">`+si_not_found_branch+`</p>`;
	}
...
function workTime(d) {
	...
	if (d.length > 6)
		var other = d.splice(6, d.length - 6, {name: "Другие",time: sum_time});
}
...
function timechartDraw(d,times, sum) {
	...
	for (var i = 0; i < times.length; i++) {
		w = (times[i] * 100) / sum;
		code += `<div class="` + d[i].color + `" title="` + d[i].name + `" style="width:` + w + `%"></div>`;
	}
		...
		html += `<div class="cdc-item" has-cross="` + cross_infos.length + `">
					...
					<div>
						...
						<div class="right">
							...
							<img src="info/icons/again/` + getChrtImg(c_b_r, d_b_r) + `.svg" />
						</div>
					</div>
					<div>
						<div class="left">
						...
						<div class="right">
							...
							<img src="info/icons/again/` + getChrtImg(c_s_r, d_s_r) + `.svg" />
						</div>
					</div>
				</div>`;
	}
	...
	if (d.data.length > 0) {
		...
		for (var i = 0; i < cb_cur.children_curr.length; i++) {
			cur_html += `<div class="ipbc-item" data-currency="`+cb_cur.children_curr[i].curr_char_code+`" onclick="showCurrency(this)">
						<div class="ipbc-icon">
							<img src="info/icons/flags/` + cb_cur.children_curr[i].curr_char_code + `.svg" />
						</div>
						<div class="ipbc-text">
...
function loadingPanel(objText,s){
	if(s){
		...
		getDOM(objText).innerHTML="<img src='/ibs/user/info/icons/loading.gif' style='margin:auto'>";
	}else{
		getDOM(objText).classList.remove("for_loader");
	}
}
function changeCurImg(t) {
	let src = "info/icons/flags/", cur = "";
...
function drawChartData(d) {
	...
	drawChartDest(curr);
	getDOM("begin_date").innerHTML = category[0].label;
	getDOM("end_date").innerHTML = category[category.length-1].label;
}
/*-----END CHART CURRENCY ------------*/
/*******************************************/
...
function goDocument() {
	go({
		url: "/ibs/user/info/payments.jsp",
		target: "modalE",
		dialogWidth: 1000,
		lock: false
	});
