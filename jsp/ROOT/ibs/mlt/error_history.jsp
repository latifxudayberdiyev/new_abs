...
<%
    ...
    String errorIdParam = request.getParameter("error_id");
    if (errorIdParam == null) errorIdParam = "";
%><t:page><%
%><t:form title="<%=si_title%>" minWidth="fill" minHeight="fill">
    ...
    <script>
        ...
        function onHistoryLoaded(list) {
            ...
            list.forEach(function(r) {
                ...
                tr.appendChild(makeCell(r.error_code != null ? String(r.error_code) : ''));
    ...
    <form id="historyForm" method="post" action="error_history.jsp" target="historyFrame" style="display:none">
        <input type="hidden" name="request" value="load_history">
        <input type="hidden" name="process_code" value="GET_HISTORY_BY_ERROR_ID">
        <input type="hidden" name="error_id" value="<%=errorIdParam%>">
    </form>
</t:form>
</t:page>
...
<%!
    ...
    static final int si_error_code    = SI("Error Code", "Код ошибки", "Xato kodi", "Error Code");
    static final int si_message_code  = SI("Message Code", "Код сообщения", "Xabar kodi", "Message Code");
    static final int si_description   = SI("Error tavsifi", "Описание ошибки", "Xato tavsifi", "Error Description");
