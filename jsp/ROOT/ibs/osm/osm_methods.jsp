...
%><t:form  minWidth="fill" minHeight="fill">
    ...
    <style>
        ...
        .osm-panel {
            ...
            padding: 10px; box-shadow: 0 1px 3px rgba(16,24,40,.05);
        }
    ...
    <script>
        /* Grid ustun indekslari (numbering ustuni 0-o'rinda):
           0=raqam 1=function_id 2=function_name 3=function_desc 4=state */
        var COL_STATE_IDX = 4;
        ...
        /* Qo'shish tugmasi ataylab yo'q: tekshiruv funksiyalarini developer
           bazaga qo'shadi, foydalanuvchi faqat formada tanlaydi. */
        function edit() {
            if (!getDOM("bEdit").disabled) {
                go({
                    url: "osm_method.jsp?process_code=GET_VALIDATE_FUNCTION",
                    param: {
                        function_id: getData(1)
                    },
                    target: "modalE",
                    dialogHeight: 420,
                    dialogWidth: 700,
    ...
    <div class="osm-tabs">
        <span class="osm-tab active"><%=lang.get(si_tab_validate)%></span>
    ...
    <div class="grid-card">
        <t:table from="Core.Action_Validate_Func_V">

            <t:field id="1" name="function_id" label="<%=si_function_id%>">
                <t:filter operator="=" />
            </t:field>
            ...
            <t:field id="3" name="function_desc" label="<%=si_function_desc%>">
                <t:filter operator="_like_" />
            </t:field>

            <t:field id="4" name="state" label="<%=si_state%>">
                <t:filter operator="="
                          optionSQL="select '<option value=''' || state || '''>' || state
                                       from (select distinct state
                                               from Core.Action_Validate_Func_V
...
<%!
    static final int si_function_id = SI("Идентификатор функции", "Funksiya identifikatori", "Funksiya identifikatori", "Function ID");
    static final int si_function_name = SI("Наименование функции", "Funksiya nomi", "Funksiya nomi", "Function Name");
    static final int si_function_desc = SI("Описание функции", "Funksiya tavsifi", "Funksiya tavsifi", "Function Description");
