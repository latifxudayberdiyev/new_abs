...
function isModal(){
	return top.location.search.indexOf('modal')==1;
}
/*JS variabledagi stylelarni olvolish uchun*/
...
/*alert() funksiyasini o`zgartirilgani
 */
/*var old_alert = window.alert;
window.alert = function (msg) {
	old_alert(replaceQGH(msg));
}*/
...
function setCSRF(t) {
	if (t.tagName === "FORM") {
		if (!t.elements["x-csrf-token"]) {
			...
			t.appendChild(inpToken);
		} else {
			t.elements["x-csrf-token"].value =  getCookie("x-csrf-token");
		}
	} else {
		t.setRequestHeader("x-csrf-token",  getCookie("x-csrf-token"));
	}
}
...
(function () {
	...
	function maskcheck(s) {
		...
		if(t.disabled) {
			t.style.background = t.currentStyle["input-background-disabled"];
                        t.style.border = t.currentStyle["input-border-disabled"];
			return true;
		}
		try {
	...
	function setError(s) {
		var t = this;
		if(s) {
			t.style.background = t.currentStyle["input-background-error"];
			t.style.border = t.currentStyle["input-border-error"];
		}
		else {
			t.style.background = t.currentStyle[t.readOnly ? "input-background-disabled" : "input-background"];
			t.style.border = t.currentStyle[t.readOnly ? "input-border-disabled" : "input-border"];
		}
	}
	...
	function maskfocus() {
		var t = this
			, f = t.check();
		if(f){ 
                   t.style.background = t.currentStyle[t.disabled || t.readOnly ? "input-background-disabled" : "input-background-focus"];
                   t.style.border = t.currentStyle[t.disabled || t.readOnly ? "input-border-disabled" : "input-border-focus"];
                }
		if(t.cm) {
			if(t.cm.type == "mask") {
				t.setValue(t.value);
	...
	function maskblur() {
		this.style.background = this.currentStyle[this.disabled || this.readOnly ? "input-background-disabled" : "input-background"];
                this.style.border = this.currentStyle[this.disabled || this.readOnly ? "input-border-disabled" : "input-border"];
		this.check();
	}
	...
	window.initElement = function (t) {
		...
		if(t.tagName == "SELECT") {
			...
			t.style.border = t.currentStyle["button-border"];
                        t.style.color = t.currentStyle["button-color"];
			t.onmouseout = function () {
				this.style.background = this.currentStyle["button-background"]
                                this.style.border = this.currentStyle["button-border"];
				this.style.color = this.currentStyle["button-color"];
			};
			t.onmouseover = function () {
				this.style.background = this.currentStyle["button-background-hover"]
                                this.style.border = this.currentStyle["button-border-hover"];
                                this.style.color = this.currentStyle["button-color-hover"];
			};
			if(t.tagName == "BUTTON") {
				t.setValue = function (v) {
			...
			if(is.undef(t.sid)) {
				...
				t.style.background = t.currentStyle[t.disabled || t.readOnly ? "input-background-disabled" : "input-background"];
                                t.style.border = t.currentStyle[t.disabled || t.readOnly ? "input-border-disabled" : "input-border"];
				t.style.color = t.currentStyle[t.disabled || t.readOnly ? "input-color-disabled" : "input-color"];
			...
			t.check = function () {
				...
				if(!k) {
					if(t.alert) alert(replaceQGH(t.alert));
					try {
						F.focus();
					} catch(ex) {
			...
			if(xhr.readyState == 4) {
				if(xhr.status == 200) {
					...
					case "error":
						...
						break;
					case "alert":
						alert(replaceQGH(xhr.responseText.trim()));
						break;
					case "json":
...
window.onload = function () {
	try {
		...
		script.src = _.URL.substring(0, ((_.URL.indexOf("/ibs/") > -1) ? _.URL.indexOf("/ibs/") : _.URL.indexOf("/ibs/")) + 1) + "tocmat_url.js";
		document.getElementsByTagName("head")[0].appendChild(script);
		if (_.getElementsByTagName("tr")[0].firstChild.className == "formTitle" || _.getElementsByTagName("tr")[0].className == "clsTitleBody") {
			_.getElementsByTagName("tr")[0].firstChild.ondblclick = function () {
				openFileNotepad()
			};
		}
	} catch (e) {
	...
	/**/
	if(typeof shbtn != "undefined") showHideButtons(shbtn);
	/**/
	...
	_.body.onkeydown = function () {
		...
		case 116: //F5 button
			if (is.def(getDOM("contents"))) {
				getDOM("contents").contentWindow.location.reload(true);
			} else {
				if (window.frameElement.src == "") {
					window.frameElement.src = window.frameElement.contentWindow.location;
				} else {
 ...
 * bu faqat developer uchun 
 * formani titliga dblclick bosa notepad++ shu jsp ochib beradi
 * tocmat_url.js fayl ichiga tomcat turgan joyini korsatiw kerak
 * var TomcatURL = "d:/Project/Tomcat8/webapps/ROOT/";
 */
function openFileNotepad() {
	try {
		...
		activeXObj.ShellExecute("c:/Program Files (x86)/Notepad++/Notepad++.exe", TomcatFileUrl, "", "Open", "1");
	} catch (e) {
		/*alert(e.message);*/
	}
}
...
function drawMenu(menu, x, y, pr) {
	...
	for(var m in menu) {
		...
		mia = "<table width=100% cellpadding=2 cellspacing=1 border=0><tr>"
		mia +="<td nowrap>" + menu[m].label;
		if(mi.tag.items) {
		...
		mi.onmouseover = function () {
			...
			if(t.tag.items) {
				if(!t.items) {
					//t.items = constMenuTimeout = setTimeout(function(){drawMenu(t.tag.items, t.offsetWidth, t.offsetTop, t.cells[0]);}, 300);
					t.items = drawMenu(t.tag.items, t.offsetWidth, t.offsetTop, t.cells[0]);
				}
			}
		};
		mi.onclick = function () {
			...
			o.removeNode(true);
			if(is.func(this.tag.action)) {
				this.tag.action();
			} else {
				eval(this.tag.action);
			}
		}
	}
	var z = 0
...
function drawTab(tab, asTab) {
	...
	/*----function showMenu----*/
	function showMenu(t) {
		if(is.def(constMenuTi) && is.def(constMenuTi.firstChild) && is.def(constMenuTi.firstChild.nextSibling) && is.def(constMenuTi.firstChild.nextSibling.innerHTML) && constMenuTi.firstChild.nextSibling.innerHTML != " &#9662;") hideDOM(constMenuTi.firstChild.nextSibling);
		...
		if(t.tag.items) {
			...
			if(is.def(constMenuTi) && is.def(constMenuTi.firstChild) && is.def(constMenuTi.firstChild.nextSibling)) hideDOM(constMenuTi.firstChild.nextSibling);
		}*/
	}
	for(var v in tab) {
		...
		ti.onmouseover = function () {
			var ti_this = this;
			constMenuTimeout = setTimeout(function(){showMenu(ti_this);}, 300);
			//showMenu(this);
		};
...
function setCookie(cname, cvalue, exdays, path) {
	if(!is.def(cvalue)){cvalue = ''}
	if(!is.number(exdays)){exdays = 1}
	if(!is.def(path)){path = '/'}
	var d = new Date();
...
function showHideButtons(p) {
	var s = ["input", "button", "select"],e;
	for (var j = 0; j < s.length; j++) {
		e = _.getElementsByTagName(s[j]);
		for (var i = 0; i < e.length; i++) {
			if (is.def(e[i].shbtn) && p.charAt(parseInt(e[i].shbtn) - 1) == "D") {
				e[i].style.display = "none";
			}
			if (is.def(e[i].shbtn) && p.charAt(parseInt(e[i].shbtn) - 1) == "E") {
				e[i].disabled = false;
			}
		}
	}
...
function vfmDrawFields(o) {
	var s, c, j, q, v, l, attr = "", d, val, req = [];
	if(is.hash(o)) {
		j = o.d;
		d = o.v;
	} else {
	...
	for(var i=0; i < j.length; i++) {
		...
		if(l.t === "text" || l.t === "password" || l.t === "email" || l.t === "number" || l.t === "date" || l.t === "file") {
			...
			for(var u=0; u < l.o_v.length; u++){
				...
				s += '<input type="checkbox" id="'+ l.n +'_'+ u +'" name="'+ l.n +'" '+ a +' value="'+ m.v +'" />';
				s += '<label for="'+ l.n +'_'+ u + '">' + m.l + '</label><br />';
			};
		} else	if(l.t === "radio-group") {
			var a, m;
			for(var y=0; y < l.o_v.length; y++){
				m = l.o_v[y];
				if(m.s === "Y" && v == '') {
					a = 'checked';
				} else if(v == m.v) {
				...
				s += '<input type="radio" id="'+ l.n +'_'+ y +'" name="'+ l.n +'" '+ a +' value="'+ m.v +'" />';
				s += '<label for="'+ l.n +'_'+ y +'">'+ m.l +'</label><br />';
			};
		} else {
	...
	function getAttr(o) {
		...
		for(var k in o) {
			v = o[k];
			switch(k) {
				case "t": a += 'type = "'+ v +'" '; 							break;	// t - type
				case "r": if(v === "Y") { a += 'r = "1" '; } else { a += ''; }	break;	// r - required
				case "m": a += 'mask = "'+ v +'" ';								break;	// m - mask
				case "n": a += 'name = "'+ v +'" ';								break;	// n - fname
				case "tl": a += 'title = "'+ v +'" ';							break;	// tl - title
				case "ml": a += 'maxlength = "'+ v +'" ';					break;	// ml - maxlength
				case "mt": a += 'multiple '; 											break;	// mt - multiple
				case "mn": a += 'min = "'+ v +'" ';								break;	// mn - min
				case "mx": a += 'max = "'+ v +'" ';								break;	// mx - max
				case "st": a += 'step = "'+ v +'" ';							break;	// st - step
			}			
			if(k === "a") {																// a - align
				switch(v) {
					case "l": s += 'text-align:left; ';							break;
					case "c": s += 'text-align:center; ';						break;
					case "r": s += 'text-align:right; ';						break;
				}
