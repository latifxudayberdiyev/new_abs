 ...
 * _.write("Hello World!");
 */
var CMS_VERSION = "4.6.0",
    __contextPath = "",
    _ = document,
    _locklayer, isShowModalDialog, clWidth;
...
function isModal() {
    return isModalWindow || getIframeLastIndex() > -1;
}
...
function go(D) {
    ...
    if (D.url.match(/^\/|http(s)?:/)) {
        ...
        isReference = is.def(D.isReference) ? D.isReference : false;
    } else {
        var o;
     ...
     * modal oynaning window.dialogArguments ning strukturasi quyidagicha
     * {arg : Arguments, opener : window(chaqirilayotgan oyna), param : param}
     */
    if (/modal/.test(D.target)) {
        if (is.undef(D.dialogWidth))
            D.dialogWidth = size(800);
        if (is.undef(D.dialogHeight))
            D.dialogHeight = size(550);
        if (is.undef(D.dialogScroll))
            D.dialogScroll = "no";
        if (is.undef(D.dialogFill))
            D.dialogFill = false;
        if (D.dialogFill) {
            D.dialogWidth = getWindowParent().innerWidth;
            D.dialogHeight = getWindowParent().innerHeight;
        }
        if (D.dialogHeight > getWindowParent().innerHeight) {
            D.dialogHeight = getWindowParent().innerHeight;
        }
        clWidth = (((clWidth) ? clWidth : _.body.clientWidth) - size(35));
        if (!isShowModalDialog && (clWidth - size(35)) < D.dialogWidth)
            D.dialogWidth = size(clWidth - size(35));
        /*if (!isShowModalDialog && (_.body.scrollHeight - size(45)) < D.dialogHeight)
        // console.log(_.body.scrollHeight);
        // D.dialogHeight = _.body.scrollHeight - size(45);
...
function showModalDialog2(D) {
    ...
    div.onmousedown = function (event) {
        if (!event.target.closest(".modal-close")) {
            div.style.filter = "drop-shadow(0 0 0 steelblue";
            div.style.transition = "filter 0s";
        }
    }
    ...
    divContent.style.width = size(D.dialogWidth) + "px";
    // divContent.style.height = size(parseInt(D.dialogHeight)) + "px";
    ...
    if (is.def(D.action) && is.undef(D.url)) {
        ...
        iframe.addEventListener("load", function () {
            ...
            if (isFilter) {
                ...
                iframe.contentWindow._.body.classList.add("referencebody");
                isReference = false;
            }
            ...
            iframe.contentWindow._.body.classList.add("removeShadow");
        });
    }
...
function fixfilterModalSize(iframe, s) {
    ...
    iframe.style.width = baseW + size(50) + "px";
    modal = iframe.closest(".modal-content");
    modal.style.width = baseW + size(50) + "px";
    modalH = getMaxHeight(baseH, size(50));
    modal.style.height = modalH + "px";
    modal.childNodes[1].style.height = modalH + "px";
    if (s) {
...
function showReference(t) {
    ...
    ref.w = (((((clWidth) ? clWidth : _.body.clientWidth)) < ref.w) ? (((clWidth) ? clWidth : _.body.clientWidth)) : ref.w);
    ref.h = (((_.body.scrollHeight) < ref.h) ? _.body.scrollHeight : ref.h);
    go({
        target: "modalE",
...
function fillRefField(t, ref, r, v, se, si) {
    ...
    if (is.def(ref.callback))
        ref.callback(r, ref, t);
    t.select();
    t.focus();
}
...
function callRequest(t) {
    ...
    if (t.getAttribute("request").charAt(0) != "{") eval("tempReq=" + window[t.getAttribute("request")]);
    else eval("tempReq=" + t.getAttribute("request"));
    const req = {...tempReq};
    var v = {
            request: req.name
        },
...
(function (e) {
    ...
    function maskcheck(s) {
        ...
        try {
            ...
            else if (t.type != "file" && t.tagName == "INPUT") t.value = t.value.trim();
    ...
    function maskkeypress() {
        ...
        if (t.cm.type == "mask") {
            ...
            if (_.selection) {
                if (t.value.length - _.selection.createRange().text.length >= t.cm.max)
                    return false;
            } else {
    ...
    */
    ...
    function maskfocus() {
        ...
        if (f)
            t.style.background = _getStyle(t, (t.disabled || t.readOnly ? "input-background-disabled" : "input-background-focus"));
        if (t.cm) {
            if (t.cm.type == "mask") {
                t.setValue(t.value);
                if (t.isEmpty()) {
    ...
    function InsDel() {
        ...
        if (t.value == "-") {
            ...
            if (is.def(t.getAttribute("beforeInsert"))) {
                if (!is.func(t.beforeInsert))
                    t.beforeInsert = eval("(" + t.getAttribute("beforeInsert") + ")");
                if (t.beforeInsert(nr) == false)
                    return;
            }
            /*initDOM qilishdan oldin sidlarni tozalab tashlaymiz*/
            clearSID(nr);
            isInsDel = true;
            if (t.getAttribute("data-plus") === "Y") newRow.replaceWith(nr);
            else tbl.replaceChild(nr, tbl.insertRow());
            nr.classList.add("insdel-animate-row");
            initDOM(nr);
            isInsDel = false;
            if (is.def(t.getAttribute("afterInsert"))) {
                if (!is.func(t.afterInsert))
                    t.afterInsert = eval("(" + t.getAttribute("afterInsert") + ")");
                t.afterInsert(nr);
            }
    ...
    window.initElement = function (t) {
        ...
        if (t.tagName == "SELECT") {
            ...
            if (is.undef(t.getAttribute("sid"))) {
                ...
                if (t.getAttribute("insdel")) {
                    ...
                    if (t.getAttribute("data-plus") !== "Y") t.value = (isInsDel) ? "-" : "+";
                    else t.value = "+";
                    //-------------------------------------
        ...
        } else if (t.tagName == "INPUT" && (t.type == "range" || t.type == "color" || t.type == "number")) {
            if (is.undef(t.getAttribute("sid"))) {
                t.setAttribute("sid", t.sourceIndex);
                t.setError = setError;
                t.check();
    ...
    window.initDOMShowHide = function (t) {
        if (t.getAttribute("showhide")) {
            var shd = t.getAttribute("showhide"),
    ...
    if (/[?&]modal/.test(_.URL)) {
        ...
        if (top == window) top.isModalWindow = false;
    }
})();
...
function evalDivSize(d, s) {
    var t = getDOM(d.id + "panel");
    let c = _.body.classList.contains("removeShadow");
    ...
    if (d.getAttribute("minWidth") != "done") {
        ...
        if (_.body.scrollWidth - _.body.clientWidth > 0) {
            d.width = "100%";
            clWidth = t.offsetWidth - (_.body.scrollWidth - _.body.clientWidth) - size(32);
            d.width = "";
        }
        if (is.undef(clWidth)) {
            clWidth = t.offsetWidth - size(32);
        } else if (clWidth > _.body.clientWidth) {
            clWidth = _.body.clientWidth - size(52);
        }
        t.style.overflow = "auto";
        t.style.width = clWidth + ((c) ? size(20) : 0);
    }
    var r = t.getBoundingClientRect();
    let clHeight = _.body.clientHeight /*- _.body.scrollHeight*/ + r.bottom; //- r.top;// - 5;
    if (clHeight > _.body.clientHeight - d.clientHeight) {
        clHeight = _.body.clientHeight - d.clientHeight + r.bottom - r.top;
    }
    if (d.getAttribute("minHeight") == "fill") {
        if (isModal()) {
            t.style.height = clHeight;
        } else {
            t.style.height = clHeight - ((c) ? 0 : size(15));
        }
    ...
    if (s == null) {
        ...
        if (t.scrollWidth - t.clientWidth < size(20) && t.scrollWidth - t.clientWidth > 0) {
...
window.onload = function (evt) {
    ...
    // request.putToHashTableURLParameters(request);
...
function drawTab(data) {
    let themeName = top._t().themeName;
    let div = "", t = "", cl = "";
    div += "<ul id='menu'>";
    ...
    function dtab(data) {
        for (var i = 0; i < data.length; i++) {
            if (is.array(data[i].items) && data[i].items.length > 0) {
                t = "<img class='expand' src=\"/ibs/user/img/" + themeName + "/svg/menu-chevron.svg\" />";
            } else {
                t = "";
            }
    ...
    if (is.def(child)) {
        ...
        if (el.parentNode.id == "menu") {
            ...
            child.childNodes.forEach(function (item) {
                ...
                if (getWidthByText(inpTxt) > elw)
                    elw = getWidthByText(inpTxt);
            });
            let pelw = 0;
            ...
            if (spacex == 0) {
                child.style.left = (pelw + size(30)) * (-1) + "px";
                child.style.width = parseInt(child.getAttribute("elw")) + size(20);
...
function printPreview(url) {
    go({
        ...
        target: "new",
        lock: false,
        arg: "left=0, top=0, width=" + (screen.availWidth - size(10)) + ", height=" + (screen.availHeight - size(70)) + ", scrollbars=1,directories=0,location=0,menubar=1,resizable=1,status=1,titlebar=0,toolbar=0"
    });
}
...
const cssVariables = {
    ...
    "button-background-hover": '#EDF3FE',
    "button-border": 'none',
    "input-border": '1px solid #EEEEF2',
    "input-border-error": '1px solid #FD4638',
    "input-background": '#ffffff',
    "input-background-focus": '#ffffff',
    "input-background-error": '#FFEDEC',
...
var HashTable = function () {
    ...
    this.putToHashTableURLParameters = function (req) {
        ...
        if (sURL.indexOf("?") > 0) {
            ...
            for (var i = 0; i < arrURLParams.length; i++) {
                ...
                if (sParam[1] != "") {
                    req.put(sParam[0], decodeURIComponent(unescape(sParam[1])));
                } else {
                    req.put(sParam[0], "");
                }
            }
...
//-------------Standardize Any Colors-----------------
function standardizeColor(c) {
    if (!c) return;
    let percent = 0, color = "";
    let colors = {
        'color-blue': '#4785EF',
        ...
        'color-gray': '#8A8A98',
        'color-green': '#24CA49',
        'color-green10': '#EAFAED',
        'color-red': '#FD4638',
        'color-red10': '#FFEDEC',
        'color-yellow': '#FF9A03',
        'color-yellow10': '#FFF5E6',
        'color-white': '#FFFFFF',
        'color-stroke': '#EEEEF2',
    ...
    for (const key of Object.keys(colors)) {
        let num = hexColorDelta(c.substring(1), colors[key].substring(1));
        if (num > percent) {
            percent = num;
            color = key;
        }
...
// select, multi-select
loadCss(`/ibs/user/util/select/vars/${top._t().themeName}.css?v=${CMS_VERSION}`);
