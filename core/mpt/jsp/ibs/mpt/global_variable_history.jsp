...
<%
	...
	String variableCode = request.getParameter("variable_code");
	try {
		ServletCallableStatement cs = new ServletCallableStatement(stored, request);
		cs.setProcedure("Core.User_Session.Put_Varchar2");
		cs.setString("i_Key", "mpt_variable_code");
		cs.setString("i_Value", variableCode);
		cs.execute();
	} catch (Exception ex) {
...
%><t:form title="<%=si_title%>" minWidth="fill" minHeight="fill">
	...
	<t:table from="mpt_print_variables_h_v">
